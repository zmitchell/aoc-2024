...
#[inline]
pub(crate) fn load_slice_to_vector(bytes: &[u8]) -> __m128i {
    unsafe { _mm_loadu_si128(bytes.as_ptr() as *const __m128i) }
}

#[allow(dead_code)]
#[inline]
pub(crate) fn bitmask_to_vector(mask: u16) -> __m128i {
    unsafe { _mm_movm_epi8(mask) }
}

#[inline]
pub(crate) fn vector_to_bitmask(mask: __m128i) -> u16 {
...
#[allow(dead_code)]
#[inline]
pub(crate) fn broadcast_to_vector(byte: u8) -> __m128i {
...
#[inline]
pub(crate) fn vector_to_slice(vector: __m128i, buffer: &mut [u8]) {
...
#[inline]
pub(crate) fn vector_to_slice_16(vector: __m128i, buffer: &mut [u16]) {
    let ptr = buffer.as_mut_ptr();
    unsafe {
        _mm_storeu_epi16(ptr as *mut i16, vector);
    }
}

#[inline]
pub(crate) fn vector_to_slice_32(vector: __m128i, buffer: &mut [u32]) {
    let ptr = buffer.as_mut_ptr();
    unsafe {
        _mm_storeu_epi32(ptr as *mut i32, vector);
    }
}
...
#[allow(dead_code)]
pub(crate) fn print_vec_u8(vector: __m128i, msg: &str) {
    let mut slice = [0; 16];
...
#[allow(dead_code)]
pub(crate) fn print_vec_u16(vector: __m128i, msg: &str) {
    let mut slice = [0; 8];
    vector_to_slice_16(vector, &mut slice);
    eprintln!("{msg}: {slice:?}");
}

#[allow(dead_code)]
pub(crate) fn print_vec_u32(vector: __m128i, msg: &str) {
    let mut slice = [0; 4];
    vector_to_slice_32(vector, &mut slice);
    eprintln!("{msg}: {slice:?}");
}
...
mod test {
    ...
    static LOOKUP_TABLE: LazyLock<Vec<PatternData>> = LazyLock::new(|| {
        let path = std::env::current_dir()
            .unwrap()
            .join("../input/day1_part1_lookup_table.dat");
        eprintln!("{}", path.display());
        load_lookup_table_from_disk(&path).unwrap()
    });
    ...
    #[test]
    fn converts_4digit_pattern2() {
        // 1111_1111_1111____
        let input = [
            b'1', b'2', b'3', b'4', b' ', b'1', b'1', b'1', b'1', b' ', b'1', b'1', b'1', b'1',
            b' ', b' ', b' ', b' ',
        ];
        let (shuffled, pat) = shuffle_slice(&input, &LOOKUP_TABLE);
        let mut output = Vec::new();
        convert_by_4digit(shuffled, &pat, &mut output);
        assert_eq!(output, vec![1234, 1111, 1111]);
    }
